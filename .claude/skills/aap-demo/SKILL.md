...
**Prerequisites**: AAP deployed, `registry.redhat.io` pull secret for OCI plugins.
...
| AO: `base_url must not resolve to a private...` / `AAP_NOT_CONFIGURED` | CoreDNS rewrite missing so AO cannot resolve the AAP route. `aap-demo wire` (or `aap-demo enable ao` / `aap-demo start`) restores the rewrite and re-wires integrations. |
