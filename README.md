...
- **16 GB RAM minimum** — default VM allocation is 16 GB (override with `CRC_MEMORY=24576 aap-demo create` for 24 GB). On hosts with 32 GB RAM or less, add temporary swap before deploy: `./scripts/enable-temp-swap.sh` (see [scripts/README.md](scripts/README.md))
