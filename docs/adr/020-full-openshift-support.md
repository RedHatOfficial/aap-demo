...
**Declined**: 2026-08-04 — Full OpenShift support requires ~20+ GB RAM and introduces
`redhat-operators`/`certified-operators` catalog dependencies not present in developer laptop
environments. The resource footprint is prohibitive for the target use case (local demo and
development). aap-demo continues to target MicroShift exclusively. The MicroShift improvements
developed alongside this ADR were merged independently.
