# Seed Nodes — Implementation Handoff
...
aap-demo nodes add 2 --image ~/rhel9.qcow2       # add VMs to existing AAP
aap-demo nodes list                                # show running VMs
aap-demo nodes remove 1                           # remove last VM
aap-demo nodes destroy                            # remove all VMs + AAP resources
```

## Files added

- `includes/seed-nodes.sh` — VM lifecycle (QEMU launch, cloud-init ISO, create/delete/list)
- `includes/seed-nodes-aap.sh` — AAP API registration (credential, inventory, hosts, ad-hoc ping)
...
- `aap-demo.sh` — `nodes` command + subcommands, `--seed`/`--image` flags, hooks into `deploy`/`destroy`/`stop`/`status`

## How VMs are created

1. Base QCOW2 image is copied once to `~/.aap-demo/nodes/base.qcow2`
2. Each VM gets a thin QCOW2 overlay (~200KB initial, copy-on-write)
3. An ed25519 SSH keypair is generated at `~/.aap-demo/nodes/ssh_key`
...
3. **Host IP detection** uses the IP on the default route interface (`route -n get default` → interface → `ifconfig <iface> inet`). This is cached at `~/.aap-demo/nodes/host_gateway_ip`. The cache must be cleared if the host IP changes (VPN, network switch, etc.).
...
A "Seed Nodes SSH Key" Machine credential is created with the generated private key. A "Seed Nodes" inventory is created in the Default org. An ad-hoc ping verifies end-to-end connectivity.

**Important:** Job templates must select the "Seed Nodes SSH Key" credential to authenticate. The ad-hoc ping run during registration uses it automatically, but user-created job templates need it assigned manually.

## Lifecycle hooks

- `aap-demo stop` — kills all seed node VMs (they're ephemeral)
- `aap-demo destroy` — destroys all seed nodes + cleans `~/.aap-demo/nodes/` before deleting the CRC cluster
- `aap-demo status` — shows a "Seed Nodes" section with running VMs

VMs are not recreated on `aap-demo start`. After a stop/start cycle, the user re-runs `aap-demo nodes add`.
...
Image path is saved to `~/.aap-demo/config` as `SEED_IMAGE=<path>` so subsequent `nodes add` commands don't need `--image`.
