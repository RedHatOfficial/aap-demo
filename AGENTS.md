...
aap-demo is a tool for deploying Ansible Automation Platform (AAP) 2.7 to OpenShift Local
(MicroShift) for development and testing.
...
aap-demo diagnose --ai   # Health check + AI-powered root cause analysis (requires Codex CLI)
...
Cross-platform destroy → deploy → addon regression runs across Mac, Linux VM, and Windows
(native PowerShell) are documented in the project skill:

- Skill: [`.cursor/skills/aap-demo-multi-host-test/SKILL.md`](.cursor/skills/aap-demo-multi-host-test/SKILL.md)
- Config template: [`.cursor/skills/aap-demo-multi-host-test/machines.example.yaml`](.cursor/skills/aap-demo-multi-host-test/machines.example.yaml)
- Orchestrator: [`scripts/multi-host-test.sh`](scripts/multi-host-test.sh)

Copy `machines.example.yaml` to `~/.aap-demo/test-hosts.yaml` (gitignored), fill in host
SSH details, then run `./scripts/multi-host-test.sh --dry-run` before any destructive test.

The orchestrator sets `AAP_DEMO_TRUST_CA=false` by default so unattended runs skip macOS sudo /
Windows elevated ingress CA import; CLI still trusts via `CURL_CA_BUNDLE`. Set `trust_ca: true`
in config for interactive browser trust during deploy.

Use `--pr NUMBER` to checkout a GitHub pull request on each host before destroy/deploy
...
**Fix**: Check `kubectl get sc` for nfs-local-rwx. If missing, re-run `aap-demo create`. If
present, check NFS server: `kubectl get pods -n nfs-storage`
...
**Fix**: Check `kubectl get pods -n aap-operator` for catalog pod. Verify pull secret exists:
`kubectl get secret redhat-operators-pull-secret -n aap-operator`
...
**Fix**: Usually resolves after operator reconciliation. If persistent, delete the pod:
`kubectl delete pod -l app.kubernetes.io/name=gateway -n aap-operator`
...
scripts/
  multi-host-test.sh           # Cross-host destroy/deploy/addon orchestrator
```

## Key Technical Details

- **SCCs**: Granted at namespace group level (`system:serviceaccounts:$NAMESPACE`) not per-SA
- **nfs-local-rwx**: In-cluster NFS server backed by topolvm PVC. Provisioner manifest uses
  `__NFS_SERVER_IP__` placeholder resolved at deploy time because kubelet can't resolve cluster
  DNS for NFS mounts on MicroShift
- **idle_aap**: Standard AAP CR field — patches `spec.idle_aap: true/false` to scale all
  components via the gateway operator
...
6. For deployment issues, check operator logs:
