#!/usr/bin/env bash
# Portal ARM Helm addon — Ansible Automation Portal on ARM clusters
# Same flow as addons/portal but uses upstream community RHDH (multi-arch) images.

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
AAP_NAMESPACE="${AAP_NAMESPACE:-${NAMESPACE:-aap-operator}}"
PORTAL_NAMESPACE="${PORTAL_NAMESPACE:-redhat-rhaap-portal-arm}"
ACTION="${1:-deploy}"
PORTAL_DIR="${HOME}/.aap-demo/portal-arm"

# Helm release name and chart
RELEASE_NAME="redhat-rhaap-portal-arm"
CHART_REPO="openshift-helm-charts/redhat-rhaap-portal"

# Default plugin version (AAP 2.7 compatible)
DEFAULT_PLUGIN_VERSION="2.2"

# Upstream community RHDH hub (amd64 + arm64) — see quay.io/rhdh-community/rhdh
...
# OAuth app name (distinct from x86 portal addon)
OAUTH_APP_NAME="${OAUTH_APP_NAME:-ansible-automation-portal-arm}"
...
cleanup() {
  echo "Disabling portal-arm addon..."
  ...
  echo "Portal-arm addon disabled"
  exit 0
}
...
check_cluster_architecture() {
  ...
  if [[ "$node_arch" != "arm64" ]]; then
    echo ""
    echo "⚠️  portal-arm targets ARM64 clusters (upstream RHDH community images)"
    echo "   This cluster is: $node_arch"
    echo "   For x86 clusters use: aap-demo enable portal"
    echo ""
    return 1
  fi
  return 0
...
check_architecture() {
  if [[ "$(detect_arch)" == "arm" ]]; then
    echo "✓ Local machine is ARM — matches portal-arm target platform"
  else
...
check_prerequisites() {
  ...
  if ! check_architecture; then
    echo ""
    read -r -p "Proceed on non-ARM cluster anyway? [y/N] " confirm
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
      echo "Aborted. Use 'aap-demo enable portal' for x86 clusters."
      exit 1
    fi
    echo ""
  fi
...
EOF
  ...
  echo "  PostgreSQL: docker.io/bitnami/postgresql:16.6.0 (multi-arch)"
}
...
verify_aap_host_url() {
  ...
  if [[ "$aap_host_url" == *".svc"* ]]; then
    ...
    echo "   Re-run: aap-demo enable portal-arm"
    return 1
  fi
...
display_success() {
  echo ""
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "✓ Portal-arm addon enabled successfully!"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo ""
  echo "Portal URL: https://$PORTAL_ROUTE"
  echo "RHDH image: ${DEFAULT_RHDH_REGISTRY}/${DEFAULT_RHDH_REPOSITORY}:${DEFAULT_RHDH_TAG}"
  echo ""
  echo "Next steps:"
  ...
  echo ""
  echo "Check status: aap-demo status portal-arm"
  echo "Disable: aap-demo disable portal-arm"
  echo ""
}
