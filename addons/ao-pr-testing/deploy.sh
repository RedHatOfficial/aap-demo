...
build_workflow_definition() {
  ...
  review_prompt="The plaibook AAP job has already performed the PR review. Its result is $dollar{run_plaibook_review.result}. Treat plaibook's structured verdict, score, findings, and run metadata as the primary review evidence. Do not redo the code review and do not use GitHub HTTP or GitHub MCP calls. Return compact JSON matching the requested result schema, preserving plaibook findings in evidence and reporting any missing run-scoped summary or blocked capability in gaps."
  verification_prompt="You are the final deployment verification stage. The plaibook review result is $dollar{run_plaibook_review.result} and the normalized review summary is $dollar{summarize_review.result}. Use only the AAP MCP and read-only OpenShift MCP tools available. Verify the plaibook job/template status and the smallest relevant local deployment evidence; do not redo the PR review, fetch GitHub data, run kubectl, fetch node or pod metrics, retrieve long logs, or perform destructive operations. A missing runner, checkout, permission, sandbox, or run-scoped result is blocked, not passed. Return compact JSON matching the requested result schema, with status passed, failed, or blocked and evidence/gaps for every affected component."
  ...
  aap_tool_selections=$(wire_ao_api GET "/integrations/$mcp_integration/tools?limit=100" | wire_ao_list_items | jq -c '[.[] | select(.name as $name | ["job_templates_retrieve","jobs_retrieve","workflow_jobs_retrieve","execution_environments_list"] | index($name)) | .id]')
  openshift_tool_selections=$(wire_ao_api GET "/integrations/$OPENSHIFT_INTEGRATION_ID/tools?limit=100" | wire_ao_list_items | jq -c '[.[] | select(.name as $name | ["resources_list","resources_get","pods_list_in_namespace","pods_get","events_list"] | index($name)) | .id]')
