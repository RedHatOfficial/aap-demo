...
apme_install_operator() {
  ...
  kubectl wait --for=condition=Available deployment/apme-operator-controller-manager \
    -n apme-operator-system --timeout=300s >/dev/null
  echo "✓ APME operator ready"
}
...
apme_apply_scaffolder_config() {
  local config
  config=$(oc process --local -f "${SCRIPT_DIR}/deploy-apme.yaml" \
    -p "NAMESPACE=${NAMESPACE}" \
    -p "CLUSTER_ROUTER_BASE=${CLUSTER_DOMAIN}" \
    -p "APME_IMAGE_TAG=${APME_OPERATOR_IMAGE_TAG:-2026.8.10}" \
    -o json \
    | jq -c '.items[] | select(.kind == "ConfigMap" and .metadata.name == "apme-scaffolder-templates")')
  [ -n "$config" ] || {
    echo "❌ APME scaffolder ConfigMap was not rendered" >&2
    return 1
  }
  echo "$config" | kubectl apply -f - >/dev/null
  echo "✓ Portal scaffolder configuration applied"
}

apme_patch_portal_config() {
  local config patched
  config=$(kubectl get configmap portal-app-config -n "$NAMESPACE" \
    -o jsonpath='{.data.app-config\.local\.yaml}')
  patched=$(printf '%s' "$config" | AAP_HOST="$AAP_HOST" yq -o=json eval \
    '.ansible.rhaap.baseUrl = strenv(AAP_HOST) | .ansible.rhaap.automationHub.baseUrl = strenv(AAP_HOST) | .auth.providers.rhaap.production.host = strenv(AAP_HOST) | .ansible.apme = {"baseUrl":"http://apme-gateway:8080","checkSSL":false,"enabled":true,"enableAi":true,"mockMode":false} | .catalog.locations += [{"type":"file","target":"/opt/app-root/src/configs/catalog/apme-register-git-repository/template.yaml","rules":[{"allow":["Template"]}]}]' - \
    | jq -r tostring)
  kubectl get configmap portal-app-config -n "$NAMESPACE" -o json \
    | jq --arg cfg "$patched" '.data["app-config.local.yaml"] = $cfg' \
    | kubectl apply -f - >/dev/null
  kubectl rollout restart deployment/redhat-rhaap-portal -n "$NAMESPACE" >/dev/null
  echo "✓ RHDH portal configured for APME"
}
...
" 2>/dev/null | tail -1)
  ...
  local repo_root local_revision project_state project_revision status
  repo_root="$(cd "${SCRIPT_DIR}/../../" && pwd)"
  local_revision=$(git -C "$repo_root" rev-parse HEAD 2>/dev/null || true)
  project_state=$(curl -sk -u "${AAP_USERNAME}:${AAP_PASSWORD}" \
    "${AAP_API}/projects/${project_id}/" 2>&1)
  project_revision=$(echo "$project_state" | jq -r '.scm_revision // empty' 2>/dev/null)
  status=$(echo "$project_state" | jq -r '.status // "unknown"' 2>/dev/null)

  if [ -n "$local_revision" ] && [ "$project_revision" = "$local_revision" ] && [ "$status" = "successful" ]; then
    echo "✓ Project already synced at ${local_revision}; skipping project update" >&2
    printf '%s\n' "$project_id"
    return 0
  fi

  local update_result update_id
  update_result=$(curl -sk -u "${AAP_USERNAME}:${AAP_PASSWORD}" \
    -X POST \
    "${AAP_API}/projects/${project_id}/update/" 2>&1)
  update_id=$(echo "$update_result" | jq -r '.id // empty' 2>/dev/null)
  if [ -z "$update_id" ]; then
    echo "❌ ERROR: Failed to start AAP project sync" >&2
    echo "$update_result" | jq -r '.detail // .error // "AAP did not return a project update ID"' 2>/dev/null || true
    return 1
  fi

  echo "Waiting for project sync (update ${update_id}, up to 5 minutes)..." >&2
  local i synced=false
  for i in $(seq 1 150); do
    status=$(curl -sk -u "${AAP_USERNAME}:${AAP_PASSWORD}" \
      "${AAP_API}/project_updates/${update_id}/" 2>&1 | jq -r '.status // "unknown"' 2>/dev/null)
    if [ "$status" = "successful" ]; then
      project_state=$(curl -sk -u "${AAP_USERNAME}:${AAP_PASSWORD}" \
        "${AAP_API}/projects/${project_id}/" 2>&1)
      project_revision=$(echo "$project_state" | jq -r '.scm_revision // empty' 2>/dev/null)
      if [ "$project_revision" = "$local_revision" ]; then
        synced=true
        echo "✓ Project synced successfully at ${project_revision}" >&2
        break
      fi
      status="successful (revision ${project_revision:-unknown})"
    elif [ "$status" = "failed" ]; then
      update_result=$(curl -sk -u "${AAP_USERNAME}:${AAP_PASSWORD}" \
        "${AAP_API}/project_updates/${update_id}/" 2>&1)
      echo "❌ ERROR: AAP project sync failed" >&2
      echo "$update_result" | jq -r '.job_explanation // .result_traceback // .detail // "See the AAP project update for details"' 2>/dev/null | tail -20 >&2 || true
      break
    fi
    if [ "$((i % 10))" -eq 0 ]; then
      echo "  Status: ${status} (waiting... $((i * 2))/300s)" >&2
    fi
    sleep 2
  done
  ...
  if [ "$synced" != true ]; then
    local final_project
    final_project=$(curl -sk -u "${AAP_USERNAME}:${AAP_PASSWORD}" \
      "${AAP_API}/projects/${project_id}/" 2>&1)
    echo "❌ ERROR: Project sync did not complete within 300 seconds" >&2
    echo "$final_project" | jq -r '
      "  Final status: \(.status // "unknown")",
      "  Last update failed: \(.last_update_failed // false)",
      "  SCM revision: \(.scm_revision // "unknown")"' 2>/dev/null || true
    return 1
  fi
...
apme_build_extra_vars_json() {
  ...
  yq -o=json eval-all 'select(fileIndex == 0) * select(fileIndex == 1)' \
    "$defaults_file" "$vars_file" \
    | jq --arg token "$openshift_token" '
      ...
      .ansible_kubernetes_context = null |
      if .github_app_private_key_content then del(.github_app_private_key_path) else . end'
}

apme_extra_vars_yaml() {
  local extra_vars_json="$1"
  # JSON is valid YAML and is accepted by the AAP extra_vars field.
  jq -c . <<<"$extra_vars_json"
}
...
apme_monitor_job() {
  ...
  local event_result event_output status_result status elapsed

  echo "Streaming AAP job output for ${label} (ID: ${job_id})..."

  for i in $(seq 1 "$max_wait"); do
    event_result=$(curl -sk -u "${AAP_USERNAME}:${AAP_PASSWORD}" \
      "${AAP_API}/jobs/${job_id}/job_events/?order_by=id&page_size=200&id__gt=${last_event_id}" 2>&1)

    if echo "$event_result" | jq -e '.results' >/dev/null 2>&1; then
      event_output=$(echo "$event_result" | jq -r '.results[] | select((.stdout // "") != "") | .stdout')
      if [ -n "$event_output" ]; then
        printf '%s\n' "$event_output"
      fi
      last_event_id=$(echo "$event_result" | jq -r '[.results[] | .id] | max // 0')
    fi

    status_result=$(curl -sk -u "${AAP_USERNAME}:${AAP_PASSWORD}" \
      "${AAP_API}/jobs/${job_id}/" 2>&1)

    status=$(echo "$status_result" | jq -r '.status // "unknown"' 2>/dev/null)
    elapsed=$(echo "$status_result" | jq -r '.elapsed // 0' 2>/dev/null)
...
\"" 2>/dev/null || true
    ...
    printf '\r  AAP status: %s (elapsed: %ss) [%d/%d]' "$status" "$elapsed" "$i" "$max_wait"
    sleep 5
  done
