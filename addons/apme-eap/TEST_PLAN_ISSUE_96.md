...
The fix ensures that on MicroShift/CRC the portal backend uses `https://<aap-route>` for in-cluster token exchange and maps the AAP route hostname to the AAP Service ClusterIP via `hostAliases`.
...
| # | Check | Pass |
|---|--------|------|
| 1 | `AAP_HOST_URL` in portal pod | `https://<aap-route>` |
...
- Use `https://` in-cluster AAP route URL and MicroShift `hostAliases` for token exchange
...
- [ ] `AAP_HOST_URL` is `https://<aap-route>` in portal pod
