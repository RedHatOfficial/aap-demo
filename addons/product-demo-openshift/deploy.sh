...
#
# Installs OpenShift and Kubernetes demos from the Ansible Product Demos collection.
...
if [ "$ACTION" = "--delete" ] || [ "$ACTION" = "delete" ]; then
  ...
  echo ""
  echo "✓ product-demo-openshift addon disabled"
  exit 0
fi
...
echo "Deploying Ansible Product Demos - OpenShift..."
echo ""
...
if apd_install_domain_demo "$DEMO_CATEGORY"; then
  echo ""
  echo "✓ OpenShift demos installed successfully!"
  echo ""
  echo "Job templates created (prefix: OPENSHIFT |):"
  echo "  - OPENSHIFT | Deploy Application"
  echo "  - OPENSHIFT | CNV Management"
  echo "  - OPENSHIFT | GitLab Setup"
  echo "  ... and more"
  echo ""
  echo "Next steps:"
  ...
  echo ""
  echo "❌ ERROR: OpenShift demo installation failed"
