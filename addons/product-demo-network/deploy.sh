...
#
# Installs Network automation demos from the Ansible Product Demos collection.
...
if [ "$ACTION" = "--delete" ] || [ "$ACTION" = "delete" ]; then
  ...
  echo "  3. Delete templates starting with 'NETWORK |'"
  echo ""
  echo "✓ product-demo-network addon disabled"
  exit 0
fi
...
echo "Deploying Ansible Product Demos - Network..."
echo ""
...
if apd_install_domain_demo "$DEMO_CATEGORY"; then
  echo ""
  echo "✓ Network demos installed successfully!"
  echo ""
  echo "Job templates created (prefix: NETWORK |):"
  echo "  - NETWORK | Backup Running Configs"
  echo "  - NETWORK | Compliance Check"
  echo "  - NETWORK | Generate Report"
  echo "  ... and more"
  echo ""
  echo "Next steps:"
  ...
  echo ""
  echo "❌ ERROR: Network demo installation failed"
